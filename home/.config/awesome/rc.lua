...
vicious.register(anothertimewidget, widgets.date, timeformat, 1, function()
  return timezone
end)
...
for i, v in ipairs({
  ...
  table.insert(timezone_menu_items, {
    v,
    function()
      timezone = v
...
vicious.register(cpupercentagewidget, vicious.widgets.cpu, function(widget, data)
...
vicious.register(memorypercentagewidget, vicious.widgets.mem, function(widget, data)
      ...
      elseif battery_percentage <= 10 then
        ...
        if not battery_notified then
          naughty.notify({
            ...
            text = string.format(
              "A current battery remaining is %d%%.\nIncidentally, lower than 5%% will immediatelly go to hibernate mode.",
              battery_percentage
            ),
            preset = naughty.config.presets.critical,
          })
...
vicious.register(netwidget, vicious.widgets.net, function(widget, data)
...
awful.rules.rules = {
  ...
  { rule = { class = "dzen2" }, properties = { sticky = true } },
  { rule = { class = "Eclipse" }, properties = { screen = 1, tag = "5" } },
  { rule = { class = "Libreoffice" }, properties = { screen = 1, tag = "8" } },
  { rule = { class = "Skype" }, properties = { screen = 1, tag = "9", ontop = true } },
  { rule = { name = terminal_class }, properties = { floating = false } },
  { rule = { class = "VirtualBox" }, properties = { screen = 1, tag = "6" } },
  {
    rule = { role = "browser" },
    properties = { screen = 1, tag = "2", border_width = 0, floating = false, opacity = 0.90 },
  },
  { rule = { class = "Zeal" }, properties = { screen = 1, tag = "9", border_width = 0, floating = false } },
}
-- }}}
...
awesome.connect_signal("battery::threshold", function()
