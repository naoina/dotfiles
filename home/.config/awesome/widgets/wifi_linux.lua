...
local function worker(format, warg)
    ...
    local f = io.popen(ipconfig .." addr show ".. warg .. " 2>&1")
    local ip = f:read("*all")
    f:close()
    ...
    if ip == nil or string.find(ip, "does not exist") then
        return winfo
    end
    ...
    winfo["{ip}"] = string.match(ip, "inet ([%d%.]+)/") or winfo["{ip}"]
    winfo["{ipv6}"] = string.match(ip, "inet6 ([%x%.:]+)/") or winfo["{ipv6}"]
