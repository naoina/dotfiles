...
local IPADDR = "PATH=$PATH:/sbin/:/usr/sbin:/usr/local/sbin ip addr show %s"

-- {{{ Wireless widget type
function wifiiw_linux.async(format, warg, callback)
  if type(warg) ~= "string" then
    return callback({})
  end
  local winfo = {}
  ...
  local function parse_ipaddr(stdout)
    winfo["{ip}"] = stdout:match("inet ([%d%.]+)/") or "N/A"
    winfo["{ipv6}"] = stdout:match("inet6 ([%x%.:]+)/") or "N/A"
  end

  spawn.easy_async_with_shell(LINK:format(warg), function(std_out, std_err, exit_reason, exit_code)
    parse_link(std_out)
    spawn.easy_async_with_shell(INFO:format(warg), function(stdout, stderr, exitreason, exitcode)
      parse_info(stdout)
      spawn.easy_async_with_shell(IPADDR:format(warg), function(stdout, stderr, exitreason, exitcode)
        parse_ipaddr(stdout)
        callback(winfo)
      end)
    end)
